...
We simulate 1000 paired MAP values, to show what the data selection described in the model-development paper does to the performance of a model that uses MAP as a predictor.
...
The correlation coefficient (` r = 0.5`) is chosen to give MAP a modest predictive ability, similar to what is presented by Davies *et al* (http://doi.org/10.1213/ANE.0000000000004121)
...
x_limits <- round(range(d$map_predictor) + c(-10, 1), 0)
...
scatter_plot_full <- ggplot(d, aes(map_predictor, map_outcome, color = hypo)) + 
    ...
    scale_y_continuous(breaks = seq(45, 105, by = 10), expand = expansion(add = c(10,10))) + 
    x_scale +
