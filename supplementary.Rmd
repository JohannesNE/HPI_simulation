...
The correlation coefficient (` r = 0.5`) is chosen to give MAP a modest predictive power, similar to what is presented by Davies *et al* (http://doi.org/10.1213/ANE.0000000000004121)
...
x_limits <- round(range(d$map_predictor) + c(-1, 1), 0)
...
scatter_plot_full <- ggplot(d, aes(map_predictor, map_outcome, color = hypo)) + 
    ...
    scale_y_continuous(breaks = seq(45, 105, by = 10), expand = expansion(add = c(1,5))) + 
    x_scale +
...
ggsave("figure3_simulation.pdf", simulation_figure, device = cairo_pdf, width = 8, height = 7)
ggsave("figure3_simulation.png", simulation_figure, bg = "white", width = 8, height = 7)
```
